use std::collections::HashMap;
extern crate termion;

const NORTH:i64 = 1;
const EAST:i64 = 4;
const SOUTH:i64 = 2;
const WEST:i64 = 3;


fn main() {
    let mut program = read_file();
    program.resize(program.len() *  10,0);

    let mut input;
    let mut pc = 0;
    let mut rb = 0;
    let mut finished = false;

    let mut game_grid = HashMap::new();

    let mut max_x = 1;
    let mut max_y = 1;


    let mut bot_location = (25,25);
    let mut current_direction = SOUTH;
    let mut initial_location = (-1,-1);
    game_grid.insert(bot_location, (4,0));


    println!("{}{}", termion::clear::All,termion::cursor::Hide );
    // let mut steps = 0;


    let mut try_left = false;
    let  mut max_steps = 0;

    
    while !finished {

        if try_left {
            input = get_new_direction(current_direction,true);
        }
        else {
            input = current_direction;
        }

        let output = do_intcode(&mut program,input,pc,rb);
        pc = output.1;
        rb = output.2;

        let old_location = bot_location;
        let new_cell = get_new_cell_info(&mut bot_location,input,output.0);

        let mut steps:i64 = 0;

        let mut temp_flag = false;
        if output.0 != 0 {
            let cell = game_grid.get(&bot_location);
            if cell == None {
               let cell:(i64,i64) = *game_grid.get(&old_location).unwrap();
               steps = cell.1 + 1; 
            }
            else {
                temp_flag = true;
                steps = cell.unwrap().1;
            }


        }


        game_grid.insert(new_cell.0,(new_cell.1,steps));



        if old_location != bot_location {
            if !temp_flag
            {
                game_grid.insert(old_location,(3,steps - 1));
            }
            else {
                game_grid.insert(old_location,(3,steps + 1));
            }
            
            game_grid.insert(bot_location,(4,steps));
        }

        use std::cmp;
        max_x = cmp::max(max_x, (new_cell.0).0);
        max_y = cmp::max(max_y, (new_cell.0).1);
        max_steps = cmp::max(max_steps,steps);
        print_bot_grid(game_grid.clone(),max_x + 1,max_y + 1);

        if try_left {
            if output.0 == 1 {
                current_direction = input;
            }
            else if output.0 == 0 {
                try_left = false;
            }
        }
        else {
            if output.0 == 1 {
               try_left = true;
            }
            else if output.0 == 0 {
                current_direction = get_new_direction(current_direction,false);
                try_left = false;
            }
        }





        if bot_location == initial_location {
            finished = true;
        }

        if output.0 == 2 {
            // finished = true;
            initial_location = bot_location;
            game_grid = HashMap::new();
            game_grid.insert(bot_location, (4,0));
            current_direction = flip_direction(current_direction);
            try_left = false;
            // max_steps = 0;
        }

    }

    // let output = do_intcode(&mut program,input,pc,rb);
    // pc = output.1;
    // rb = output.2;

    // println!("{:?}",output.0);

    // let output = do_intcode(&mut program,input,pc,rb);
    // pc = output.1;
    // rb = output.2;

    // println!("{:?}",output.0);
    // println!("{:?}", steps);
    println!("robot {:?}", game_grid.get(&bot_location));
    println!("{:?}", max_steps);

    
    println!("{}",termion::cursor::Show );
}

fn flip_direction(direction:i64) -> i64{
    let new_direction;

    match direction {
            NORTH => new_direction =  SOUTH,
            EAST => new_direction =  WEST,
            SOUTH => new_direction =  NORTH,
            WEST => new_direction =  EAST,
            _ => new_direction =  -1000,
    }  

    new_direction
}

fn get_new_direction(direction:i64, left:bool) -> i64 {
    let new_direction;
    if left {
        match direction {
            NORTH => new_direction = WEST,
            EAST => new_direction = NORTH,
            SOUTH => new_direction = EAST,
            WEST => new_direction = SOUTH,
            _ => new_direction = -1000,
        }  
    }
    else {
        match direction {
            NORTH => new_direction = EAST,
            EAST => new_direction = SOUTH,
            SOUTH => new_direction = WEST,
            WEST => new_direction = NORTH,
            _ => new_direction = -1000,
        }
    }


    new_direction
}


fn get_new_cell_info(bot_location: &mut(i64,i64),direction: i64,cell_type: i64) -> ((i64,i64),i64){
    // let new_cell;
    let new_cell_location;
    match direction {
        NORTH => new_cell_location = (bot_location.0,bot_location.1 - 1),
        EAST => new_cell_location = (bot_location.0 + 1,bot_location.1),
        SOUTH => new_cell_location = (bot_location.0,bot_location.1 + 1),
        WEST => new_cell_location = (bot_location.0 - 1,bot_location.1),
        _ => new_cell_location = (-100,-100),
    }

    if cell_type != 0 {
        *bot_location = new_cell_location;
    }

    (new_cell_location,cell_type)
}

fn print_bot_grid(game_grid: HashMap<(i64,i64),(i64,i64)>,max_x:i64,max_y:i64) {

    print!("{}", termion::cursor::Goto(0, 2));

    for y in 0..max_y {
        for x in 0..max_x {
            let tile_id = game_grid.get(&(x,y));
            if tile_id == None {
                // print!(" ");
                print!("     ");
                continue;
            }
            match tile_id.unwrap().0 {
                0 => print!("#####"),
                1 => print!("..."),
                2 => print!("O"),
                3 => print!("|{:03}|",tile_id.unwrap().1),
                4 => print!("DDDDD"),
                _ => print!("E"),
            }
            // match tile_id.unwrap().0 {
            //     0 => print!("#"),
            //     1 => print!("."),
            //     2 => print!("O"),
            //     3 => print!("*",),
            //     4 => print!("D"),
            //     _ => print!("E"),
            // }
        }
        println!();
    }

    println!();
    println!();
}


fn do_intcode(viktor: &mut Vec<i64>,program_input: i64,pc:usize, rb:usize) -> (i64,usize,usize){
        let mut program_counter = pc;
        let output;
        let mut relative_base = rb;
        let input = program_input;

        loop {
            let instruction = viktor[program_counter];
            let mut program_increment = 4;

            let instruction = format!("{:05}",instruction);
            let op_code = &instruction[3..];

            if op_code == "99"{
                // return (-1,0);
                // return -1;
                // break
                println!("exit");
                return (-100,program_counter,relative_base);
            }

            let mode3 = &instruction[0..1];
            let mut address3 = 0;
            if program_counter + 3 < viktor.len()
            {
                if mode3 == "0"{
                    address3 = viktor[program_counter+3] as usize;
                }
                else if mode3 == "2" {
                    address3 = (viktor[program_counter+3] + relative_base as i64) as usize;
                }
                else {
                    println!("unrecognised parameter mode p3");
                }
            }



            if op_code == "01" {
                let values = get_values(instruction,viktor.to_vec(),program_counter,relative_base);

                viktor[address3] = values.0 + values.1;
            }
            else if op_code == "02" {
                let values = get_values(instruction,viktor.to_vec(),program_counter,relative_base);

                viktor[address3] = values.0 * values.1;
            }
            else if op_code == "03"
            {
                // println!("test");
                // let input = 3;
                // let input;
                let mode1 = &instruction[2..3];

                let address1;
                if mode1 == "0" {
                    address1 = viktor[program_counter+1] as usize;
                }
                else if mode1 == "2" {
                    address1 = (viktor[program_counter+1] + relative_base as i64) as usize;
                }
                else {
                    address1 = 0;
                    println!("error unrecognised parameter mode");
                }

                program_increment = 2;
                viktor[address1] = input;
            }
            else if op_code == "04"
            {
                output = get_value(instruction,viktor.to_vec(),program_counter, relative_base);
                // println!("Output: {:?}", output);
                program_increment = 2;
                program_counter += program_increment;
                return (output,program_counter,relative_base);
            }
            else if op_code == "05"
            {
                let values = get_values(instruction,viktor.to_vec(),program_counter, relative_base);
                if values.0 != 0{
                    program_counter = values.1 as usize;
                    continue;
                }
                program_increment = 3;
            }
            else if op_code == "06"
            {
                let values = get_values(instruction,viktor.to_vec(),program_counter, relative_base);
                if values.0 == 0{
                    program_counter = values.1 as usize;
                    continue;
                }
                program_increment = 3;
            }
            else if op_code == "07"
            {
                let values = get_values(instruction,viktor.to_vec(),program_counter, relative_base);
                if values.0 < values.1 {
                    viktor[address3] = 1;
                }
                else {
                    viktor[address3] = 0;
                }
            }
            else if op_code == "08"
            {
                let values = get_values(instruction,viktor.to_vec(),program_counter, relative_base);
                if values.0 == values.1 {
                    viktor[address3] = 1;
                }
                else {
                    viktor[address3] = 0;
                }
            }
            else if op_code == "09"
            {
                let value = get_value(instruction,viktor.to_vec(),program_counter, relative_base);
                let temp_base = relative_base as i64 + value;
                relative_base = temp_base as usize;
                program_increment = 2;
            }
            else {
                println!("????");
                println!("Something went wrong: Unrecognised op code");
                println!("pc {:?}",program_counter);
                println!("inst {:?}",instruction);
            }

            program_counter += program_increment;
        }
    // output
}

//taken from luke smith
//https://github.com/lukesmith-bjss/advent-of-code-2019/blob/master/day_2/src/main.rs
fn read_file() -> Vec<i64> {
    include_str!("data.txt").split(',')
        .map(|i| i.parse::<i64>().unwrap())
        .collect()
}

fn get_value(instruction:String,viktor: Vec<i64>,program_counter: usize,relative_base: usize) -> i64 {
    let mode1 = &instruction[2..3];

    let value;

    if mode1 == "0" {
        let address1 = viktor[program_counter+1] as usize;
        value = viktor[address1];
    }
    else if mode1 == "1" {
        value = viktor[program_counter+1];
    }
    else if mode1 == "2" {
        let address1 = (viktor[program_counter+1] + relative_base as i64) as usize;
        value = viktor[address1];     
    }
    else {
        println!("error unrecognised parameter mode");
        value = -1;
    }

    value
}

fn get_values(instruction:String,viktor: Vec<i64>,program_counter: usize,relative_base: usize)  -> (i64,i64) {
    let mode1 = &instruction[2..3];
    let mode2 = &instruction[1..2];

    let value1;
    let value2;

    if mode1 == "0" {
        let address1 = viktor[program_counter+1] as usize;
        value1 = viktor[address1];
    }
    else if mode1 == "1" {
        value1 = viktor[program_counter+1];
    }
    else if mode1 == "2" {
        let address1 = (viktor[program_counter+1] + relative_base as i64) as usize;

        value1 = viktor[address1];     
    }
    else {
        value1 = -1;
        println!("error unrecognised parameter mode for p1");
    }

    if mode2 == "0" {
        let address2 = viktor[program_counter+2] as usize;
        value2 = viktor[address2];

    }
    else if mode2 == "1" {
        value2 = viktor[program_counter+2];
    }
    else if mode2 == "2" {
        let address2 = (viktor[program_counter+2] + relative_base as i64) as usize;
        value2 = viktor[address2];     
    }
    else {
        println!("error unrecognised parameter mode for p2");
        value2 = -1;
    }

    (value1,value2)
}